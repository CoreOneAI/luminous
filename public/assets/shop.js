(function(){
  ...
  const imgFor = (p) => {
    ...
    return 'https://images.unsplash.com/photo-1556228453-efd1f4a7fae9?auto=format&fit=crop&w=800&q=60';
  };
  ...
  function filterByMode(items, mode){
    ...
    switch(mode){
      ...
      case 'antiaging':
        return full.filter(p => anyMatch(((p.name||'') + ' ' + (p.category||'')), SYN.antiaging) || /retinol|peptide|collagen|vitamin c|anti[- ]?aging|wrinkle|firm|spf|sunscreen/i.test((p.name||'') + ' ' + (p.category||'')));
      case 'kbeauty':
        return full.filter(p => anyMatch(((p.name||'') + ' ' + (p.category||'')), SYN.kbeauty));
      case 'nailcare':
        return full.filter(p => anyMatch(((p.name||'') + ' ' + (p.category||'')), SYN.nails));
      case 'accessories':
